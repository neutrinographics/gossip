...
///
/// [ConnectionError] represents expected failures during Nearby Connections
/// transport operations:
/// - Connection failures (not found, disconnected)
/// - Handshake failures (timeout, invalid data)
/// - Send failures (network issues)
///
/// Applications should observe the error stream from `NearbyTransport`
...
final class HandshakeInvalidError extends ConnectionError {
  ...
  @override
  String toString() =>
      'HandshakeInvalidError(endpointId: $endpointId, message: $message)';
}

/// Error when failed to send bytes over Nearby Connections.
...
///
/// Used by services to report errors to observers.
typedef ConnectionErrorCallback = void Function(ConnectionError error);
