...
void main() {
  ...
  group('DigestRequest budgeting (H4)', () {
    ...
    test('rotation covers every stream over successive rounds', () async {
      ...
      expect(
        covered,
        equals(streams.toSet()),
        reason:
            'round-robin rotation must eventually advertise every '
    ...
    test(
      'a single stream whose version vector alone exceeds the budget is '
  ...
  group('DigestResponse budgeting (H4)', () {
    test(
      ...
      () async {
        ...
        expect(
          responseSizes.single,
          lessThanOrEqualTo(300),
          reason:
              'the response must also fit the budget, even when the '
  ...
  group('DigestResponse rotation (OBS-3)', () {
    test('successive over-budget responses rotate the fitted window instead of '
      ...
      expect(
        firstStreams.length,
        lessThan(streams.length),
        reason:
            'the reply must be over budget for this pin to mean '
