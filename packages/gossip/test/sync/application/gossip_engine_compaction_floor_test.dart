...
void main() {
  ...
  group('requester side', () {
    ...
    test('a solicited response with a floor is adopted: survivors merge, '
        'no error, range never re-requested', () async {
      final h = GossipEngineTestHarness();
      ...
      () async {
        ...
        expect(
          response.entries
              ...
              .toSet(),
          equals({'author-a#4', 'author-a#5', 'author-b#7', 'author-b#8'}),
          reason:
              'each author\'s survivors must be served independently '
        ...
        expect(
          ...
          equals(6),
          reason:
              'authorB\'s floor must reflect ITS OWN, DIFFERENT prune '
  ...
  group('compaction mid-sync race', () {
    test(
      ...
      () async {
        ...
        final all = await responder.entryRepository.getAll(channelId, streamId);
        ...
        expect(
          ...
          equals(3),
          reason:
              'the response must carry the floor computed at HANDLE '
        ...
        expect(
          ...
          isNotEmpty,
          reason:
              'a follow-up round must make progress — no wedge left '
