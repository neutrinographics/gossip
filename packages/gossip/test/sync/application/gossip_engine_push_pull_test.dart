...
void main() {
  ...
  group('GossipEngine push-pull reciprocation', () {
    test('a DigestRequest from a peer that is ahead triggers a reciprocal '
        'DeltaRequest — each exchange is push-pull', () async {
    ...
    test('a DigestRequest from an in-sync peer produces no reciprocal '
        'DeltaRequest (only the digest response)', () async {
      final h = GossipEngineTestHarness();
      final peer = h.addPeer('peer1');
      h.createChannel('ch1', streamIds: ['s1']);
      h.startListening();
    ...
    test('a DigestRequest carrying channels we do not share produces no error '
        'spam and still reciprocates for the shared channel', () async {
      final h = GossipEngineTestHarness();
      final peer = h.addPeer('peer1');
      h.createChannel('ch1', streamIds: ['s1']);
      h.startListening();
    ...
    test('a listen-only (not running) engine serves the digest but does NOT '
        'reciprocate — a paused node must not pull', () async {
      final h = GossipEngineTestHarness();
      final peer = h.addPeer('peer1');
      h.createChannel('ch1', streamIds: ['s1']);
