...
void main() {
  test(
    'an append racing an in-flight compaction on the same stream: neither '
    'is lost, and sequence allocation stays monotonic',
    () async {
      ...
      for (var i = 0; i < 3; i++) {
        await service.appendEntry(channelId, streamId, Uint8List.fromList([
          i,
        ]));
      }
      ...
      expect(
        ...
        isTrue,
        reason: 'the interleaved append must not be lost to a concurrent '
      ...
      expect(
        ...
        reason: 'no duplicate/colliding sequence numbers — a compaction '
      ...
      expect(
        latest,
        equals(4),
        reason: 'the high-water mark must reflect the append even though '
