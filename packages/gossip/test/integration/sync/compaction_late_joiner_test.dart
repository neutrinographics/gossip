...
void main() {
  ...
  test(
    'late joiner vs compacted responder: floor adoption, content-checked, '
    'and traffic quiesces after convergence (COR3-1)',
    () async {
      ...
      expect(
        result,
        isNotNull,
        reason: 'compaction must actually prune something, or this pin '
            'proves nothing',
      );
      ...
      expect(
        ...
        isTrue,
        reason: 'the late joiner must converge despite the responder having '
      ...
      expect(
        ...
        equals([4, 5]),
        reason: 'B must hold EXACTLY the surviving entries — a content '
      ...
      expect(
        lateCount,
        lessThan(earlyCount),
        reason: 'the futile-resend-loop symptom must stay dead: idle '
  ...
  test(
    'transitive floor propagation: C joins via B only and never talks to A '
    '(adoptVersionFloor raises the adopter\'s own servable floor)',
    () async {
      ...
      expect(
        ...
        isTrue,
        reason: 'C must converge with B even though C never talks to A '
      ...
      expect(
        ...
        equals([4, 5]),
        reason: 'C must hold exactly the entries B was able to serve, '
  ...
  test(
    'returning peer below the floor: reconnect after the responder has '
    'compacted past the peer\'s last-known position',
    () async {
      final network = await TestNetwork.create(['a', 'b']);
      await network.setupChannel(
        channelId,
      ...
      expect(
        ...
        equals({1, 2, 3, 7, 8}),
        reason: 'B keeps what it already had (1..3) and adopts the floor '
      ...
      expect(
        errors,
        isEmpty,
        reason: 'skipping the unobtainable 4..6 range via floor adoption '
  ...
  test(
    'prune-all then new appends: a floor-only peer with no entries still '
    'accepts new contiguous history',
    () async {
      final network = await TestNetwork.create(['a', 'b']);
      await network.setupChannel(
        channelId,
