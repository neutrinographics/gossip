...
// codec by injection instead of constructing the composite `ProtocolCodec`
// inline. This pins the resulting cross-context behavior: a frame from the
// OTHER context's message family decodes to null (per-context `MessageCodec`
// answers "not mine") and must be silently ignored — no thrown error, no
// reply sent, no state mutated. Before the split, `ProtocolCodec` decoded
// every frame regardless of family and the engine's message-type dispatch
// simply had no matching branch; this test is the regression proof that the
