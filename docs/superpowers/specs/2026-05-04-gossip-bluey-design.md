...
- **Peripheral-side NodeId limitation (KNOWN ISSUE).** During implementation, we discovered that bluey's `PeerClient` (the server-side view of a connected central) does not expose the central's `ServerId`. It only exposes a `Client.id` — a transient platform UUID (random per session on iOS, MAC-derived on Android). The current `BlueyPortImpl` synthesizes a `NodeId` from this transient ID. Consequences on real hardware: (1) when device A connects to device B as a central, B's gossip layer sees A under a synthetic ID rather than A's true `NodeId` — mesh sync is broken because both sides identify each other inconsistently; (2) the synthetic ID doesn't survive a disconnect-reconnect cycle on the peripheral side. **Workaround for v2:** add an in-band one-shot NodeId announcement — the central writes its own `NodeId` as the first frame on the data characteristic; the peripheral reads it and updates its `NodeId` mapping before delivering subsequent frames to the gossip layer. This restores a small handshake in exchange for symmetric identity. The integration tests in v1 use a fake `BlueyPort` that propagates real NodeIds, so they pass; real-hardware testing was out of scope for v1.
