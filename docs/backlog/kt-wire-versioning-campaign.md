...
| Adopting a peer's claim about our own authorship as a sequence floor (Kotlin side) | **Closed** by Batch KT-B (2026-08-29) — ported alongside the contiguity guard; see the Kotlin repo's `docs/superpowers/plans/2026-08-29-kt-batch-b-sync-depth.md`. |
| Splitting large answers into pages, and fitting summaries to a size budget, on the Kotlin side | This item's scope, above — after the format settles. The wire spec's out-of-scope section records the reassignment. **Raised in urgency by the stalled-range port** (2026-09-01 review): until kt paginates, every stalled-range probe makes the responder re-ship the entire stalled range in one unbounded frame that the requester rejects wholesale — suppression caps that at the 10-minute backoff cadence (versus every round before), but pagination is what makes a probe cheap. |
...
| Making the Kotlin library's relay-based health probing work at all | **Ruled a retirement, not a repair** (owner, 2026-09-01): [Retire indirect health probing from both libraries](kt-retire-indirect-probing.md), per the [decision record](../superpowers/specs/2026-09-01-swim-slimdown-decision.md). The defect was found by Batch KT-D (2026-08-30) — the relay blocks the queue its own answer must arrive through — and its resolution is deletion on both sides; the once-untranslatable Dart relay scenario is now obsolete rather than pending. |
...
| Removing the Kotlin library's dead incarnation machinery | **KT-E scope** (owner ruling, 2026-09-01): the refutation scaffolding (`Peer.incarnation`, the registry's increment/update methods, the persisted local incarnation and its create-time restore) has zero production callers and nothing on the wire can drive it — Dart deliberately omits refutation and documents why. Removal touches the server's `LocalNodeRepository` implementation; ship it with the next submodule bump. See the [divergence register](kt-normalize-twin-divergences.md)'s SWIM-incarnation row. |
| Normalizing version-vector explicit-zero emission on the Kotlin side | **KT-E scope** (owner ruling, 2026-09-01, reversing the register's "likely permanent"): the Kotlin library adopts the Dart twin's construction-time zero-dropping so the two encoders become symmetric and byte-comparisons of either side's output against the other's vectors stop tripping on `"node":0`. Existing captured goldens stay untouched — they are truthful about the deployed artifact; the change ships with fresh vectors. See the [divergence register](kt-normalize-twin-divergences.md)'s explicit-zero row for the full history. |
