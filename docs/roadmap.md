...
4. **Stability hardening batch (Kotlin)** — **cleared to start 2026-09-14**:
...
- ◐ **Medium** — [Stop the Kotlin library from treating cancellation as a failure](backlog/kt-cancellation-swallowed.md) · five suspend-carrying catch-alls swallow the coroutine cancellation signal (nine at spec time; two vanished with the scheduler rewrite, two codec catches are unreachable and left alone) — folded into the coordinator-lifecycle batch, which restructures the same sites
