...
- ◐ **High** — [Make the Kotlin library's indirect health probing actually work](backlog/kt-swim-indirect-probing-inert.md) · the relay step of failure detection is inert — the relay blocks the queue its answer must arrive through, so it always times out; a one-way-deaf peer is marked unreachable where Dart keeps it reachable — batched with the two items below; [rulings drafted](superpowers/specs/2026-09-01-receive-loop-lifecycle-rulings.md), awaiting review
