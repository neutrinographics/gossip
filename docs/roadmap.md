...
went first — shipped as Heroku v51 on 2026-09-21, with its design
audit's one exposed-surface finding closed the same evening as v52, and
**accepted by the first real room on v52 on 2026-09-22**: eight phones,
`wait=` 0 / 120 ms, no timeouts, no warnings
([2026-09-22 measurement](audits/2026-09-22-production-meeting-v52.md)) —
ahead of the wire work. Work
...
7. ☑ **Measure** — **done 2026-09-17**: the first meeting on v48 (170
   ...
   **Accepted on v52, 2026-09-22** ([report](audits/2026-09-22-production-meeting-v52.md)):
   eight phones at twice the v50 frame rate read `wait=` 0 / 120 ms, with
   zero ping timeouts, warnings, holes or no-session sends, and the
   compaction tick after the room pruned; traffic rose to ~1 MB out per
   phone per minute because the server now keeps up, which is the
   digest-scoping number to beat.
...
8. ☑ **Finish the Dart half of the relay retirement** — **done 2026-09-18**
   (merged 2026-09-18 as 9ada5fd, PR #16; rulings page
...
- ☐ **Medium** — [Only tell a peer about the groups you both belong to](backlog/engine-scope-digests-to-shared-groups.md) · digests advertise every channel a node holds, including its own user channel; measured on a mixed Android/iOS pair as 19 unusable channel ids × 22 rounds (~6.4 KB/exchange), and in the 2026-09-15 meeting as ~97 % of 710 KB out per phone per minute (the v48 meeting of 2026-09-17 measured ~300 KB per phone, the v50 meeting of 2026-09-21 ~700 KB again, the v52 acceptance room of 2026-09-22 ~1 MB because the server now keeps up; digest share to be re-measured on v52) — wasted airtime, log noise, and group/account ids disclosed to unrelated peers
...
- ☑ **Medium** — [Stop the server reading a whole stream to answer a per-author question](backlog/server-entry-repository-full-stream-reads.md) · **done 2026-09-21** in opendoor-api PR #25 (R2 of the inbound-queue spec): entries-since is a SQL predicate and the batch append looks up only its own keys; the status query's per-stream byte-size walk is the one leftover, on the audit follow-ups item · originally: the batch append and the entries-since query each load the entire stream and filter in memory, ~150 times a minute in a meeting; push the predicates into SQL, and bound the transaction helper's IO dispatcher while there (final review of the meeting-fixes PR, 2026-09-16)
- ☑ **High** — [Keep the server's inbound queue under a second in a meeting](backlog/server-inbound-queue-under-load.md) · **done 2026-09-22**: accepted by the first real room on v52 — eight phones, `wait=` 0 / 120 ms median / max at twice the v50 frame rate, zero ping timeouts, zero warnings, compaction pruning after the room ([report](audits/2026-09-22-production-meeting-v52.md)) · **shipped 2026-09-21**: opendoor-api PR #25 (merge b703e9f, released as Heroku v51 at 20:48 local) — stream marks served from memory by a decorator over the Postgres repository (lazy load under one mutex, write-through after commit, forget on failure, per-stream generation that forgets on a retirement race), entries-since as a SQL predicate, batch-key duplicate check; one-phone tunnel run read zero marks selects after warm-up and `wait=` in single-digit milliseconds; design audit on the PR (no Critical/Major) with its fixes landed; **done when the next real meeting reads `wait=` under a second at eight to ten phones** · measured 2026-09-21 at eight phones: the queue fills within ninety seconds and holds a 23–27 s wait all meeting, so a page change takes two minutes to reach the room and the server drops sockets through its 30 s ping timeout (the pong is read by the loop stuck behind the full queue; six of eight phones in one minute); about 90 ms of work per inbound message today, most of them summaries that merge nothing — measure where it goes, then cheaper replies (full-stream reads first) and, if needed, per-group parallel handling in the Kotlin engine (needs a ruling); acceptance is `wait=` under a second in a real meeting
...
- ☑ **Medium** — [Give the sync-check its own question to ask, instead of the engine's storage](backlog/server-sync-check-query-port.md) · **done 2026-09-21** (opendoor-api PR #26, merge 989fc15, released as Heroku v52 at 21:36 local: `StreamProgressQuery` port + one-row Postgres adapter, route no longer injects the engine's repository; validated with direct requests through the tunnel, one marks row per lookup, zero cache loads) · the open `/sync/check` route reaches the engine's sixteen-method storage port with caller-supplied names, and since the marks cache that is one process-wide lock and an unbounded table; one narrow query interface on Postgres, the route injects that (audit CA1-2 and the PR's automated review, 2026-09-21) — owner's call 2026-09-21: the next server PR after the inbound-queue one
- ☐ **Low** — [Small follow-ups from the inbound-queue design audit](backlog/server-audit-follow-ups.md) · dyno-name guard for the one-process assumption, a real-cancellation test for the cache's guards, the cache's rule stated as the port's with an LSP test and named raise helpers, one aggregate statement for stream byte size, the flaky reconnect-chain WebSocket test
...
- ☑ **High** — [Retire indirect health probing from both libraries](backlog/kt-retire-indirect-probing.md) · Kotlin half shipped in gossip-kt ea0d51c (PR #8, 2026-09-15) — relay handler counts and ignores, indirect phase became a grace window racing the late answer, sender guard ported; Dart half shipped 2026-09-18 merged as 9ada5fd (PR #16) ([rulings](superpowers/specs/2026-09-18-dart-relay-retirement-rulings.md) approved; ADR-004/012 amended, SWIM renamed away, grace-window race adopted); both halves done; the app pin bump is item 8's tail.
