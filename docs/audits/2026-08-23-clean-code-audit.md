...
## Remediation — Batch H (2026-08-27)

Commits `b20aac9..428eaa0` + this commit, branch `cc5-batch-h`. Six tasks (H1–H6), three formal fix rounds (H1: a guide-truth gap the first pass's greps missed; H3: six findings incl. the undocumented CompositeRetention const break; H5: a real hang the reviewer reproduced) plus H2's one-token slip fix (a stray audit-ID prefix left in a test comment).

**Baseline closed:** MIN-3 (silent oversized-push and no-reachable-partner skips now logged), MIN-4 (`StreamCompacted` now emitted from `ChannelService.compactStream`, silent on a no-op pass), MIN-5 (`Hlc.tryParse` returns null over an out-of-range `ArgumentError` too, not just `FormatException`), MIN-6 (`TimeBasedRetention`/`CountBasedRetention`/`CompositeRetention` constructors gained guards; const invocation lost on `TimeBasedRetention` and `CompositeRetention` — CHANGELOG-recorded), MIN-7 (`StreamConfig`, `ChannelDelta`, `MergeResult`, `BufferOverflowOccurred` deleted), MIN-8 (`VersionVector` copies its input map and normalizes explicit zeros away — wire-safety verified: no production path ever constructs a zero-valued vector), MIN-9 (`EventStream.getAll()` returns typed `List<LogEntry>`), MIN-13 (`InMemoryTimePort` timers honor their interval; non-positive intervals now rejected, after a review-caught hang — see below). **OBS-9 closed:** `entriesForAuthorAfter` deleted; `CompactionResult.oldBaseVersion`/`newBaseVersion` collapsed to one `baseVersion` field, proven contractually equal (`removeEntries` never regresses the vector); the engine's dead `selectRandomPeer` fossil had already gone in Batch E.

**OBS-8 dispositioned kept-with-rationale:** `ChannelAggregate` stays exported — it is `ChannelRepository` implementers' SPI (OpenDoorApp's Hive-backed repository is a real, working consumer); the class doc now states the mutation-bypass risk instead of hiding the type. **MIN-22 verified already closed** by Batch A's doc sweep — re-checked against today's source: `coordinator.dart`'s `events` doc now points at the `SyncEvent`/`MembershipEvent` families instead of naming four stale examples, and `rtt_estimate.dart`'s default doc says 500 ms, matching the constant.

**Ledger ruling — test-name shorthand scoped out:** the `(H4)`/`(G3)` batch-key suffixes in test *names* are an established naming convention and sit outside CC5-40's no-shorthand-in-comments rule; whether they survive is deferred wholesale to the comment-hygiene pass, with no renames landing in this batch.

**CC5-18's deferred removals executed:** `PeerRepository.findReachable`/`exists`/`count` deleted. **Routed items landed:** `PeerDirectory.selectRandomPartner` and `GossipEngine.clearPendingRequests` deleted as dead surface; `FailureDetector.timePort` made private and `.rttTracker` marked `@visibleForTesting`; stack traces threaded at the coordinator's three remaining drop sites (`_handleEntriesMerged`, `probeNewPeer`, and the compaction scheduler's `onTickError`/`onSchedulingError`); both deferred pins landed (`PendingPullTracker`'s exact-timeout boundary, the delta-side "can never fit" message). `implementing-ports.md` had its gossip-round selection description, its EntryRepository hot-path list (naming the since-deleted `entriesForAuthorAfter`), and its `PeerRepository` section all brought back to the settled contract. `PeerRegistry.selectRandomReachablePeer` was swept as an H1 follow-on — zero callers or tests anywhere in the monorepo. `architecture.md` was found wholly stale (pre-reorg: a since-abandoned layer-first structure, a facade marked "not yet implemented") and routed to the comment-hygiene backlog item for a supersede-or-delete decision rather than a line edit.

**Consumer-visible changes, CHANGELOG-recorded:** `entriesForAuthorAfter`/`findReachable`/`exists`/`count` removed, `BufferOverflowOccurred` removed, `EventStream.getAll()` now typed, `CompactionResult`'s field collapse, both retention classes' const loss, `StreamCompacted` now emitted, and `InMemoryTimePort.advance()` now honoring its interval (schedulePeriodic rejects non-positive ones).

**Review-caught defects fixed in-batch:** the new `advance()` loop hung forever on a zero or sub-millisecond interval on the public `InMemoryTimePort` (reviewer reproduced the hang via a killed test run) — fixed with an `ArgumentError` guard and red pins for both the zero and sub-millisecond cases; `implementing-ports.md` still mandated the already-deleted `entriesForAuthorAfter` contract after H1's first pass, because the batch's citation greps never covered `docs/guides` — fixed in H1's own fix round, with this batch's closing wave re-verifying no further guide drift remained.
...
- **DI1** — batch-key suffixes (`(H4)`, `(G3)`, `(WIRE4-3)`, …) stripped from 53 test/group names across `test/`; string-literal-only edits, every assertion and callback body byte-identical.
